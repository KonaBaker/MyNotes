...
注意:命名空间都是具有external 连接属性的,只是匿名的命名空间产生的**UNIQUE_NAME**在别的文件中无k法得到,这个唯一的名字是不可见的.
...
`sizeof` 是编译期操作，如果作用在运行时才会分配的对象，比如vector，那么返回的是它本身的大小，而不是它管理的数据。

```c++
栈上的 vector 对象 (sizeof 看到的)        堆上的实际数据
┌─────────────────────────┐              ┌──────────────────────┐
│ T*     data_ptr  (8字节) │──────────→  │ Vertex[0]            │
│ size_t size      (8字节) │              │ Vertex[1]            │
│ size_t capacity  (8字节) │              │ Vertex[2]            │
└─────────────────────────┘              │ ...                  │
   sizeof(vertices) = 24                 └──────────────────────┘
   (固定值，与元素数量无关！)               ← 这才是你想要的大小
```
...
对于`std::array`是可以用`sizeof`的

---

### meta

元编程就是 让编译器写代码。写代码让代码生成代码本身。

- 泛型编程。代码通用性，用模板写一份适配多种类型的带按摩。
- 模板元编程。关注编译期计算，用模板把计算提前到编译阶段。
