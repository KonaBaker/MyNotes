...
- 一个 constexpr **变量**是一个**编译时**完全确定的常数。**必须**完成常量初始化。
  ...
  ```c++
  constexpr std::vector<Vertex> vertices = { ... };
  ```

  是错误的，`std::vector`会调用`operator new`去堆上动态分配内存，是运行期行为。

  如果想要编译器常量数组可以使用`std::array`
  ...
  前提是`Vertex`是字面类型（构造函数是`constexpr`，成员也是字面类型）。

- 一个 constexpr **函数**至少对于某一组实参可以在编译期间产生一个**编译期**常数。**包含只读语义**
