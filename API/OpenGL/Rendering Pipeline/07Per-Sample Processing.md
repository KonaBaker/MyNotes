...
> 在不开启MSAA的时候per-sample = per-fragments,开启mutisample的时候，最小粒度的处理单位就是per-sample.
...
fragment可以和buffer中的颜色进行混合。

启用了`GL_FRAMEBUFFER_SRGB`，则混合之前会转换为线性rgb。

## srgb conversion

启用了`GL_FRAMEBUFFER_SRGB`，写入的image如果是srgb格式，就从线性转换为srgb。

## dithering

如果写入的image精度不如输入值（float值被写入归一化整数的image），需要进行向上或者向下取整。

当抖动被禁用的时候，向上还是向下取决于具体实现。此时会出现锯齿/无细微渐变/色带。

可以使用抖动`GL_DITHER`来消除这种现象。
...
（一种特殊的混合？自己理解）通过位运算和framebuffer中的颜色进行混合。

仅适用于整数格式（是否归一化都可以）以及未启用srgb的image format。

## write mask

【详细见 write mask】

对于一些buffer的写入或者其中的一些分量进行屏蔽。
