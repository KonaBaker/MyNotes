...
一个 set 里有多个 binding,每个 binding 包含一个或多个 descriptor（多个的情况对应数组）。一个 buffer 可以被多个 descriptor 共享,一个descriptor指向某个buffer的某个范围。
...
**这里的binding和vertex的binding不同**

- 顶点输入：vertex buffer binding -> attribute location -> shader 输入字段
- 资源输入：descriptor set + descriptor binding -> shader 里的 [[vk::binding(binding, set)]]

两者使用的是不同的资源槽位以及不同的bind
